...
export const signup = (email, password) => {
  return async dispatch => {
    const response = await fetch('https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=AIzaSyA6c4WNyJPhAmVComPmwLl4vM1CElrjJXg',
    {
      method:'POST',
      headers:{
...
export const login = (email, password) => {
  return async dispatch => {
    const response = await fetch('https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=AIzaSyA6c4WNyJPhAmVComPmwLl4vM1CElrjJXg',
    {
      method:'POST',
      headers:{
        'Content-Type': 'application/json'
